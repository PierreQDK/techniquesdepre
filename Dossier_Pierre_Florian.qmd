...
```{r}
# *****************************************
# Modélisation LED Holt-Winters automatique
# *****************************************

# LED = Lissage exponentiel double : niveau + tendance (sans saisonnalité)
...
# Graphique de l'ajustement des valeurs lissées
plot(m$fitted[,1])

# *****************************************
# Prévision sur un horizon h = 12
# *****************************************

# Prévision à 12 périodes avec IC à 80 % et 95 %
library(forecast)
fit <- forecast(m, h = 12)
...
# Affichage de l’objet complet de prévision
show(fit)

# Prévision ponctuelle pour les 12 mois
prevf <- fit$mean
show(prevf)
...
#### 2.4.1 - AIC et AICc de ADAM ETS+ARIMA
```{r}
aic_adam_ets_arima <- AIC(fit_adam_ets_arima)
cat("AIC du modèle ADAM ETS+ARIMA :", round(aic_adam_ets_arima, 2), "\n")

# AICc du modèle ADAM ETS + ARIMA
aicc_adam_ets_arima <- AICc(fit_adam_ets_arima)
