...
# === 1. Créer une séquence de dates pour la prévision sur 12 mois ===
h <- 12
last_date <- as.Date("2019-12-01")  # Dernier point de ta série
dates_future <- seq(from = last_date %m+% months(1), by = "month", length.out = h)

# === 2. Créer le data frame combiné avec tes objets de prévision déjà créés ===
df_all <- tibble(
  Date = rep(dates_future, times = 6),
  Prévision = c(as.numeric(prev_adam$mean),
                as.numeric(prev_adamarima$mean),
                as.numeric(prev_arima$mean),
                as.numeric(prev_hw$mean),
                as.numeric(prevssarima$mean),
                as.numeric(prev_ces$mean)),
  Modèle = rep(c("ADAM.ETS", "ADAM.ETS.ARIMA", "ARIMA", "Holt.Winters", "SSARIMA", "CES"), each = h)
)

# === 3. Ajouter la série réalisée si tu veux la comparer ===
serie_reelle <- window(cac40_ts, start = c(2019, 1))
dates_reelles <- seq(from = as.Date("2019-01-01"), by = "month", length.out = length(serie_reelle))

df_real <- tibble(
  Date = dates_reelles,
  Prévision = as.numeric(serie_reelle),
  Modèle = "réalisée"
)

# === 4. Combiner prévisions et réalisée ===
df_plot <- bind_rows(df_all, df_real)

# === 5. Tracer le graphique final ===
ggplot(df_plot, aes(x = Date, y = Prévision, color = Modèle)) +
  geom_line(size = 1) +
  labs(
...
# === 1. Créer la série tronquée jusqu'à fin 2017 ===
cac40_train <- window(cac40_ts, end = c(2017, 12))  # Données jusqu’à déc 2017
cac40_test <- window(cac40_ts, start = c(2018, 1), end = c(2018, 12))  # Réalisé en 2018

# === 2. Modèles estimés sur la période d'apprentissage ===
fit_adam_ets <- auto.adam(cac40_train, model = "ZZZ", lags = c(1,12), select = TRUE)
...
# === 3. Prévisions sur 2018 ===
h <- 12
prev_adam <- forecast(fit_adam_ets, h = h)
...
# === 4. Créer l'axe temporel correspondant à l’année 2018 ===
dates_2018 <- seq(from = as.Date("2018-01-01"), by = "month", length.out = h)

# === 5. Assembler les données dans un data frame tidy ===
df_forecast <- tibble(
  Date = rep(dates_2018, times = 7),
  Valeur = c(as.numeric(prev_adam$mean),
             as.numeric(prev_adamarima$mean),
             as.numeric(prev_arima$mean),
             as.numeric(prev_hw$mean),
             as.numeric(prev_ssarima$mean),
             as.numeric(prev_ces$mean),
             as.numeric(cac40_test)),
  Modèle = rep(c("ADAM.ETS", "ADAM.ETS.ARIMA", "ARIMA", "Holt.Winters", "SSARIMA", "CES", "réalisée"), each = h)
)

# === 6. Tracer le graphique final de comparaison ===
ggplot(df_forecast, aes(x = Date, y = Valeur, color = Modèle)) +
  geom_line(size = 1) +
  labs(
    title = "Prévisions vs Réalisé pour l’année 2018 — Modèles de prévision du CAC 40",
    x = "Date",
    y = "Indice CAC 40"
  ) +
  theme_minimal(base_size = 13) +
