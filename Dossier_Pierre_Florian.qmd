...
cac40_prev_ts <- ts(data = cac40_prev$cloture, start = c(2019, 01), frequency = 12)
...
fit_ssarima_2018 <- auto.ssarima(log_cac40_ts, lags = c(1, 12),
                                 orders = list(ar = c(3, 3), i = c(2), ma = c(3, 3)), select = TRUE)
fit_ces_2018 <- auto.ces(log_cac40_ts)
...
```{r}
# MSE
mse_adam      <- mean((log_cac40_prev - prev_adam$mean)^2)
mse_adamarima <- mean((log_cac40_prev - prev_adamarima$mean)^2)
mse_arima     <- mean((log_cac40_prev - prev_arima$mean)^2)
mse_hw        <- mean((log_cac40_prev - prev_hw$mean)^2)
mse_ssarima   <- mean((log_cac40_prev - prev_ssarima$mean)^2)
...
log_cac40_prev <- as.numeric(log_cac40_prev)

mse_naive <- mean((log_cac40_prev - naive_forecast)^2)
...
errors_cumul <- tibble(
  ...
  Erreur_carrée = c(
    ( log_cac40_prev- prev_adam$mean)^2,
    (log_cac40_prev - prev_adamarima$mean)^2,
    (log_cac40_prev - prev_arima$mean)^2,
    (log_cac40_prev - prev_hw$mean)^2,
    (log_cac40_prev - prev_ssarima$mean)^2
  ),
...
err_adam       <- log_cac40_prev - prev_adam$mean
err_adamarima  <- log_cac40_prev- prev_adamarima$mean
err_arima      <- log_cac40_prev - prev_arima$mean
err_hw         <- log_cac40_prev - prev_hw$mean
err_ssarima    <- log_cac40_prev - prev_ssarima$mean

# Prévision naïve : moyenne constante
naive_forecast 
err_naive      <- log_cac40_prev - naive_forecast
...
df_rolling <- tibble(
  ...
  Réalisé = as.numeric(log_cac40_prev)
)
