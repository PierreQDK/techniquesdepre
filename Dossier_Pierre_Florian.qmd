...
```{r}
# Classe time series
log_cac40_ts <- ts(data = cac40$cloture, start = c(2000, 01), frequency = 12)
cac40_prev_ts <- ts(data = cac40_prev$cloture, start = c(2000, 01), frequency = 12)
...
Le test combiné de type WO (combined_test) a été réalisé pour détecter des effets saisonniers ou calendaire multiples dans la série log_cac40_ts. La statistique de test globale est de 0, avec des p-values respectives de 1, 1 et 0,1039 pour les différents sous-tests. La p-value finale étant légèrement supérieure à 10 %, on ne rejette pas non plus l’hypothèse nulle d’absence d’effet saisonnier ou calendaire. Ainsi, ce test ne met pas en évidence de structure saisonnière marquée ni d’effet lié à des facteurs calendaires dans la série analysée.
...
stats <- basicStats(diff_log_cac40_ts)
show(stats)
```
...
```{r}
library(EnvStats)
rosnerTest(diff_log_cac40_ts, k = 10, alpha = 0.05)
```
...
fit_adam_ets <- auto.adam(
  ...
  model = "ZAZ",         # sélection automatique de la structure ETS
...
### 2.6 - CES (Complex Exponential Smoothing)
...
mod_ces <- auto.ces(
  log_cac40_ts, # série temporelle utilisée
)
...
```{r}
# Définir les séries d'apprentissage et de test
cac40_train_2018 <- window(log_cac40_ts, end = c(2018, 12))
cac40_test_2019  <- window(log_cac40_ts, start = c(2019, 1), end = c(2019, 12))

# Estimation des modèles sur la base d'apprentissage (jusqu’à 2018)
fit_adam_2018 <- auto.adam(cac40_train_2018, model = "ZZZ", lags = c(1, 12), select = TRUE)
fit_adamarima_2018 <- auto.adam(cac40_train_2018, model = "ZZZ", lags = c(1, 1, 12),
                                orders = list(ar = c(3, 3), i = c(2), ma = c(3, 3)), select = TRUE)
fit_arima_2018 <- auto.arima(cac40_train_2018)
fit_hw_2018 <- HoltWinters(cac40_train_2018, gamma = FALSE)
fit_ssarima_2018 <- auto.ssarima(cac40_train_2018, lags = c(1, 12),
                                 orders = list(ar = c(3, 3), i = c(2), ma = c(3, 3)), select = TRUE)
fit_ces_2018 <- auto.ces(cac40_train_2018)
...
prev_ces_2019        <- forecast(fit_ces_2018, h = h)
...
df_forecast_2019 <- tibble(
  ...
  Valeur = c(
    ...
    as.numeric(prev_ces_2019$mean),
    as.numeric(cac40_test_2019)
  ),
...
ggplot(df_forecast_2019, aes(x = Date, y = Valeur, color = Modèle)) +
  geom_line(size = 1.2) +
  labs(
    title = "Prévisions mensuelles pour l’année 2019 : comparaison des modèles",
    x = "Date",
    y = "Indice CAC 40"
  ) +
...
cac40_test  <- window(log_cac40_ts, start = c(2019, 1), end = c(2019, 12))
...
for (i in 0:11) {
  ...
  train_window <- window(log_cac40_ts, end = train_end)
  test_value <- window(log_cac40_ts, start = c(start_year, start_month + i), end = c(start_year, start_month + i))
...
```{r}
# Graphique des prévisions sur l'année 2018 (voir code précédent pour df_forecast_2018)

ggplot(df_forecast_2018, aes(x = Date, y = Valeur, color = Modèle)) +
  geom_line(size = 1.1) +
  labs(
    title = "Évolution des prévisions des différents modèles – Année 2018",
    subtitle = "Comparaison avec la série réalisée (CAC 40)",
    x = "Date",
    y = "Indice CAC 40"
  ) +
  theme_minimal(base_size = 13) +
