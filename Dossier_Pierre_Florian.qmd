...
Les prévisions des modèles naïf et ADAM_ETS sont identiques sur l'ensemble de l'échantillon, produisant à chaque date exactement les mêmes valeurs. En conséquence, leurs erreurs de prévision sont strictement égales.
Le test de Diebold-Mariano, qui évalue la performance relative de deux modèles en analysant la variance des différences d'erreurs, ne peut être appliqué ici : la variance étant nulle, aucune supériorité statistique ne peut être établie. C'est pourquoi le test n'a pas pu être réalisé entre les deux modèles.
...
fittbats = tbats(yy)
prevtbats <- forecast(fittbats,1)
forc = as.numeric(fc$mean)
format[i,1] <- forc
...
prevision <- map_dbl(1:1, function(i) {
  
  serie_realisations <- serie[i:(estimation-1+i)]
